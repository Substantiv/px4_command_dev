                                                     ...
                                                     Eigen::Vector3d& vel_setpoint)
{
    ...
    // 位置模式是对位置进行控制，速度模式为对速度进行控制，
    // 在位置模式下，控制器调节无人机的速度或加速度，使无人机朝着某个期望的位置移动，最终到达该位置并保持稳定。
    // 在速度模式下不需要关心无人机的位置，因此相较于位置控制无比例控制

    if((_Reference_State.Sub_mode & 0b10) == 0) //xy channel
    {
        // xy位置模式: 速度设定点是通过位置误差（期望位置与当前无人机位置的差）乘以比例增益Kp_xy并加上期望速度来计算
    ...
    {
        // z位置模式: 速度设定点是通过位置误差（期望位置与当前无人机位置的差）乘以比例增益Kp_z并加上期望速度来计算
